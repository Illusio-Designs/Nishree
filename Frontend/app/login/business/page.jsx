...
// Consumers use the email/Google login instead.
export default function BusinessLoginPage() {
  ...
  const verify = async (e) => {
    ...
    setLoading(true);
    try {
      const data = await otpLogin({ phone, role, otp });
    ...
    >
      {step === 'phone' ? (
        ...
        <form onSubmit={verify} className="space-y-4">
          ...
          <Input label="OTP" name="otp" icon={SecurityLockIcon} placeholder="6-digit code" value={otp} onChange={(e) => setOtp(e.target.value)} required />
