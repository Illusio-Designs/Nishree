...
export const setupDatabase = async () => {
    try {
		...
		const baseTableNames = ['User', 'Category', 'Attribute', 'Product', 'Settings', 'ShippingFee', 'SeoMetadata', 'Coupon'];

		// Helper to safely sync a model and recover from tablespace issues
		const safeSync = async (model, options = { alter: true, hooks: false }) => {
			try {
				await model.sync(options);
			} catch (err) {
				const errno = err?.parent?.errno || err?.original?.errno;
				// ER_TABLESPACE_EXISTS (1813) recovery: drop and force recreate
				if (errno === 1813) {
					try {
						let rawTableName = null;
						if (typeof model.getTableName === 'function') {
							const t = model.getTableName();
							rawTableName = typeof t === 'string' ? t : (t?.tableName || null);
						} else if (model.tableName) {
							rawTableName = model.tableName;
						}
						if (rawTableName) {
							console.warn(`Tablespace exists for ${rawTableName}. Attempting drop and recreate...`);
							// Try disable FK checks and drop table
							await sequelize.query('SET FOREIGN_KEY_CHECKS = 0;');
							await sequelize.query(`DROP TABLE IF EXISTS \`${rawTableName}\`;`);
						...
						console.error('Failed to recover from tablespace error:', dropErr);
						throw err;
					}
				} else if (err?.name === 'SequelizeUnknownConstraintError' || /does not exist/i.test(err?.message || '')) {
					// Ignore attempts to drop non-existent constraints during alter sync
					const tableName = (typeof model.getTableName === 'function' && model.getTableName()?.tableName) || model.tableName || 'unknown';
					const constraint = err?.constraint || 'unknown';
					console.warn(`Skipping unknown constraint during sync: ${tableName}.${constraint}`);
					return;
				} else {
					throw err;
				}
		...
		for (const modelName of baseTableNames) {
			if (models[modelName]) {
				...
				await safeSync(models[modelName], { alter: true, hooks: false });
			} else {
		...
		const dependentTableNames = [
			...
			'Country', 'State', 'City', 'ContactMessage', 'Wishlist'
		];
